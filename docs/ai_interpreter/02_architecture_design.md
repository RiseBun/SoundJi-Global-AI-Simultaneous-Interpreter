...
- 课程主题和术语表输入。
- 实时字幕视图。
- 知识树视图：准备阶段展示大标题树，播放过程中增量补充小标题；数据层保留核心观点、关键原句和时间轴引用。
- 浮动知识树视图：全屏视频左右黑框区域显示透明背景白字树，支持拖动、横向拉宽、左/右锚点切换和纵向增长。
- 会后复盘视图。
- 导出按钮。
...
知识树在数据层保留完整证据结构，在浮动展示层只展示学习时最有用的架构层级。这样可以避免全屏观看时被过细文本干扰。

| 展示形态 | 使用场景 | 视觉策略 | 信息层级 |
|---|---|---|---|
| `architecture_graph` | 文档、答辩、复盘页 | Mermaid 或节点图，展示 Root、Branch、Subtopic 的结构关系 | Root / Branch / Subtopic |
| `growing_code_tree` | 全屏视频左侧黑框 | 类代码目录树，采用连续主干、`├─`/`└─` 短横节点、等宽白字 | Root / Branch / 当前 Subtopic |
| `living_text_tree` | 全屏视频左下浮动真树 | 主干自下而上生长，枝干末梢显示 `[+]`/`[-]`，文字贴近枝干并自动换行 | Root / Branch / 当前 Subtopic |

当前 demo 的默认选择：

- 列表树使用 `growing_code_tree`，视觉上更接近工程目录树，适合高密度扫描。
- 真树使用 `living_text_tree`，视觉上更有“课程结构生长”的感觉，适合展示差异化。
- 浮动视图默认不显示时间戳，不显示过细的三级分点，只展示标题核心。时间戳、关键原句和证据引用保留在导出和数据对象中。

### 4.7.2 浮动黑边交互契约

```text
FloatingKnowledgeTreeView
  -> anchor: left_black_bar by default
  -> background: transparent
  -> text: white monospace
  -> draggable: true
  -> resizable_width: true
  -> resize_handle: right border
  -> auto_wrap: true
  -> avoid_video_content: true
  -> default_density: title_core_only
```

交互规则：

- 默认出生点在视频左侧黑框。列表树靠左上；真树靠左下并整体上移，避免压到底部控制条。
- 面板右侧有可拖拽边框，用户可左右拉伸宽度。
- 列表树拉宽后，文本换行减少；缩窄后，标题在边界内强制换行。
- 真树拉宽后，枝干末梢随边界向右移动，枝干长度和文字换行同步变化。
- 真树的 `[+]`/`[-]` 放在枝干末梢圆点附近，不放在标题文字前，减少文字干扰。
- 自动折叠只发生在一级大标题层级；二级小标题不做自动折叠。
- 浮动树不能越过黑框进入视频主体区域，除非用户手动拖动。
...
- FloatingKnowledgeTreeView 检查：必须声明 `architecture_graph`、`growing_code_tree`、`living_text_tree` 三种展示模式，且 `floating`、`draggable`、`resizable_width` 为 true。
...
| 知识树展示 | 架构图 + 连续主干代码树 + 真树文字树 | 架构图便于讲解结构，代码树适合高密度扫描，真树文字树适合展示“课程结构随播放生长”的差异化。 |
...
| 浮动树遮挡视频内容 | 默认锚定视频左侧黑框区域；支持拖动、横向拉宽和自动换行；树过高时只自动折叠前一个一级大标题，保留当前学习分支。 |
